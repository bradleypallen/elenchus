...
gh label create commitment --repo "$REPO" --color 0E8A16 --description "An asserted proposition (left side of state)" 2>/dev/null && echo "  ✓ commitment" || echo "  · commitment (exists)"
gh label create denial --repo "$REPO" --color B60205 --description "A denied proposition (right side of state)" 2>/dev/null && echo "  ✓ denial" || echo "  · denial (exists)"
...
echo "  - Commitments (green): Propositions you assert (left side of state)"
echo "  - Denials (red): Propositions you reject (right side of state)"
echo ""
echo "Terminology (following Restall 2004):"
echo "  - State [X : Y]: describes what you assert (X) and deny (Y)"
echo "  - Sequent X ⊢ Y: claims that state [X : Y] is incoherent"
echo ""

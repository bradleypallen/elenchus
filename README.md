...
The dialectical structure draws from medieval *obligationes* (formal disputation games), Roberts' Questions Under Discussion framework from linguistics, and Restall's bilateral sequent calculus from "Multiple Conclusions" (2005).
...
READMEs for each of these repositories were generated by Elenchus by requesting "Generate a README.md file in the dialectical repository summarizing the current state of the dialectic." Viewing the issues in the GitHub web interface will display the detail of the dialectic's current state.
